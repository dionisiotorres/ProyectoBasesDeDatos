...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 24-03-2019 a las 06:17:15
...
--
-- Estructura Stand-in para la vista `forma03`
--
CREATE TABLE IF NOT EXISTS `forma03` (
...
--
-- Estructura Stand-in para la vista `horasdisponibles`
--
CREATE TABLE IF NOT EXISTS `horasdisponibles` (
`Edificio` varchar(45)
,`NumeroAula` varchar(45)
,`HoraDisponible` time
);
-- --------------------------------------------------------
...
--
-- Estructura para la vista `forma03`
--
DROP TABLE IF EXISTS `forma03`;
...
--
-- Estructura para la vista `horasdisponibles`
--
DROP TABLE IF EXISTS `horasdisponibles`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `horasdisponibles` AS select `ed`.`nombre` AS `Edificio`,`au`.`numero` AS `NumeroAula`,`sc`.`horaFin` AS `HoraDisponible` from ((`edificio` `ed` join `aula` `au` on((`au`.`Edificio_idEdificio` = `ed`.`idEdificio`))) join `seccion` `sc` on((`sc`.`Aula_idAula` = `au`.`idAula`))) where ((not(`sc`.`horaFin` in (select `ss`.`horaInicio` from ((`seccion` `ss` join `aula` `au` on((`au`.`idAula` = `ss`.`Aula_idAula`))) join `edificio` `ed` on((`ed`.`idEdificio` = `au`.`Edificio_idEdificio`)))))) and (`au`.`estado` = 'A'));
